...
class LilygoDualCanContractTests(unittest.TestCase):
    ...
    def test_cache_is_bounded_default_off_and_uses_ack_replay(self):
        ...
        self.assertIn("#define MOT_OFFLINE_CACHE_MAX_BYTES 131072", cache)
        self.assertIn("constexpr size_t MAX_BYTES = MOT_OFFLINE_CACHE_MAX_BYTES", cache)
    ...
    def test_lilygo_uses_isolated_vehicle_identity(self):
        ...
        self.assertIn('vehicleId = LILYGO_DEFAULT_VEHICLE_ID', header)
        board = (ROOT / "firmware/lilygo-t-a7670/include/board_config.h").read_text(encoding="utf-8")
        self.assertIn('#define LILYGO_DEFAULT_VEHICLE_ID "pioneer-lilygo"', board)
