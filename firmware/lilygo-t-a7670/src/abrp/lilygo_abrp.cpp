...
bool sendLilygoAbrpTelemetryNow()
{
    ...
    if (WiFi.status() != WL_CONNECTED) {
        lastSuccess = false;
        lastHttpCode = 0;
        lastMessage = "ABRP transport unavailable: WiFi not active";
        failCount++;
        return false;
    }

    // NTP best-effort. ABRP can still accept telemetry without utc, but with
    // valid WiFi this usually gives us a proper timestamp quickly.
    if (!timeValid()) {
    ...
    if (lastSuccess) {
        successCount++;
        lastSendMs = millis();
        Serial.printf("ABRP: HTTP %d OK\n", lastHttpCode);
    } else {
        failCount++;
        Serial.printf("ABRP: HTTP %d failed: %s\n", lastHttpCode, lastMessage.c_str());
    }
...
String lilygoAbrpStatusJson()
{
    ...
    json += "\"transport\":\"WiFi\",";
    json += "\"transportAvailable\":" + String(WiFi.status() == WL_CONNECTED ? "true" : "false") + ",";
