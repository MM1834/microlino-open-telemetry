...
| CLOUD-008 | No user/device claim lifecycle | ONB-001.B1 adds a locally tested guarded admin invitation/assignment workflow; claim records, transfer, replacement and recovery APIs remain absent | Controlled beta assignment improves, but self-service binding and lifecycle recovery are not yet safe | Complete B1 apply validation, then implement reviewed B2 claim transaction and B3 lifecycle |
