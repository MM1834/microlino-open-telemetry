...
| CLOUD-008 | No user/device claim lifecycle | ONB-001.B1 adds a locally tested guarded admin invitation/assignment workflow; B2 claim and B3 replacement/transfer lifecycle designs exist but APIs remain absent | Controlled beta assignment improves, but self-service binding and lifecycle recovery are not yet implemented | Implement the reviewed B2 transaction and B3 idempotent lifecycle only after Change Set and credential-delivery review |
